...
class Program
{
    ...
    static long startingTime = 1000000000;
    ...
    const float Y_MOVE_AMOUNT = 0.55f; // Amount to move down
        ...
        if (enemyMoveCounter + MOVE_FREQUENCY >= MOVE_FREQUENCY - 1)
        {
            foreach (var enemy in enemies)
            {
                ...
                if (enemy.X <= 0 || enemy.X >= width - 1)
                {
                    ...
                    enemy.Y = (int)(enemy.Y + Y_MOVE_AMOUNT);
                }
        ...
        for (int j = enemies.Count - 1; j >= 0; j--)
        {
            ...
            for (int i = bullets.Count - 1; i >= 0; i--)
            {
                if (bullets[i].X == enemies[j].X && bullets[i].Y == enemies[j].Y)
                {
                    ...
                    float damage =  5  +  5  *  player.GetLevel()  -  enemies[j].Defense * 2;
                    ...
                    if (enemies[j].Health <= 0)
                    {
                        ...
                        score += (byte)random.Next(100, 150);
                    }
                    break;
                }
            }
...
class Player
{
    ...
    public int GetLevel()
    {
        ...
        return (int)Math.Round((float)Exp / 5);
    }
